...
let carModels = inventory.map(function(item) { return item["car_model"]; });
let carModelsSorted = carModels.sort();
console.log(carModelsSorted);

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a new array from the dealer data containing only the car years and log the result in the console.
let carYears = inventory.map(function(item) { return item["car_year"]; });
console.log(carYears);

// ==== Challenge 5 ====
// The car lot manager needs to find out how many cars are older than the year 2000. Using the carYears array you just created, find out how many cars were made before the year 2000 by populating the array oldCars and logging it's length.
let oldCars = carYears.filter(function(x) {
  return x < 2000;
});
...
let BMWAndAudi = [];
let bmwStringVal = new String("BMW").valueOf();
let audiStringVal = new String("Audi").valueOf();
for(i = 0; i < inventory.length; i++){
if(inventory[i].car_make.valueOf() === bmwStringVal || inventory[i].car_make.valueOf() === audiStringVal)
